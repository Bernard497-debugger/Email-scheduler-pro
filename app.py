#!/usr/bin/env python3
"""
Auto Scheduled Email Sending App - FINAL WORKING VERSION
API key confirmed working - Ready for Render
"""
...
# ==================== CONFIGURATION ====================
# YOUR WORKING API KEY (confirmed working from test)
BREVO_API_KEY = ''  # PASTE YOUR FULL KEY HERE
BREVO_FROM_EMAIL = 'botsile55@gmail.com'
...
def schedule_email():
    ...
    try:
        ...
        flash(f'⏰ Auto-scheduler checks every 30 seconds', 'info')
    except ValueError:
...
def test_brevo():
    ...
    if success:
        flash(f'✅ Test email sent! Check your inbox. Message ID: {msg_id[:20]}...', 'success')
    else:
        flash(f'❌ Failed: {msg}', 'error')
...
def scheduler_status():
    ...
    <body style="font-family:Arial;padding:20px">
        ...
        <p>✅ API Key: WORKING</p>
        <p>✅ Scheduler Thread: RUNNING</p>
        <p>📊 Scheduled emails: {scheduled}</p>
...
<body>
<div class="header"><div class="nav"><div class="logo">🚀 Email Scheduler Pro <span class="badge">API WORKING</span></div><div class="nav-links">{% if session.user_id %}<a href="/dashboard">Dashboard</a><a href="/logout">Logout</a>{% else %}<a href="/login">Login</a><a href="/register">Register</a>{% endif %}</div></div></div>
<div class="hero"><h1>⚡ Schedule Emails Automatically</h1><p>Brevo API Connected ✓ Auto-scheduler Active ✓</p>{% if not session.user_id %}<a href="/register" class="btn">Get Started Free</a>{% endif %}</div>
...
<div style="text-align:right;margin-bottom:20px"><a href="/test_brevo" class="test-btn">🔧 Test API</a> <a href="/scheduler_status" class="test-btn" style="background:#0369a1">📊 Status</a></div>
...
if __name__ == '__main__':
    ...
    print(f"✅ Auto-scheduler: ACTIVE (checks every 30s)")
