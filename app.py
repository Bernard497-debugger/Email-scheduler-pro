#!/usr/bin/env python3
"""
Auto Scheduled Email Sending App - Save as scheduler.py (NOT email.py)
Using Brevo API - WITH BEAUTIFUL ANIMATIONS
"""
...
# Brevo API Configuration - UPDATE THESE!
BREVO_CONFIG = {
    'api_key': '',
    'from_email': 'botsile55@gmail.com',
...
def send_email_via_brevo(to_email, subject, body, user_email=None):
    try:
        headers = {'api-key': BREVO_CONFIG['api_key'], 'Content-Type': 'application/json'}
        html_content = f"""
        <!DOCTYPE html>
        <html>
        ...
        return False, f"Error: {response.json().get('message', 'Unknown')}", None
...
def send_scheduled_email(email_id):
    ...
    try:
        ...
        if user['emails_sent_this_month'] >= PLANS[user['plan']]['emails_per_month']:
            ...
            db.commit()
            return False
        success, msg, brevo_id = send_email_via_brevo(email['recipient_email'], email['subject'], email['body'], user['email'])
        if success:
...
def register():
    if request.method == 'POST':
        ...
        if get_user_by_email(email):
            ...
            return redirect(url_for('login'))
        db = get_db()
        db.execute('INSERT INTO users (email, password, last_reset) VALUES (?, ?, ?)', (email, hash_password(password), datetime.now().date().isoformat()))
        db.commit()
...
def dashboard():
    ...
    return render_template_string(DASHBOARD_TEMPLATE, user=user, scheduled_emails=emails, plan=plan, plan_name=user['plan'], usage_percentage=percent, remaining_emails=get_remaining_emails(user), remaining_scheduled=get_remaining_scheduled(user))
...
def schedule_email():
    ...
    try:
        ...
        if scheduled <= datetime.now():
            ...
            return redirect(url_for('dashboard'))
        db = get_db()
        db.execute('INSERT INTO scheduled_emails (user_id, recipient_email, subject, body, scheduled_time) VALUES (?, ?, ?, ?, ?)', (user['id'], recipient, subject, body, scheduled.isoformat()))
        db.commit()
...
def test_brevo():
    user = get_user(session['user_id'])
    success, msg, _ = send_email_via_brevo(user['email'], '✨ Brevo API Test', f'Test email sent at {datetime.now()}\n\nYour email scheduler is working perfectly! 🎉', user['email'])
    flash('✅ Test email sent! Check your inbox.' if success else f'❌ Failed: {msg}', 'success' if success else 'error')
    return redirect(url_for('dashboard'))
...
<body>
    <div class="header"><div class="nav"><div class="logo">🚀 Email Scheduler Pro <span class="badge">Brevo API</span> <span class="badge free-badge">300/day Free</span></div><div class="nav-links">{% if session.user_id %}<a href="/dashboard">Dashboard</a><a href="/logout">Logout</a>{% else %}<a href="/login">Login</a><a href="/register">Register</a>{% endif %}</div></div></div>
    <div class="hero"><h1>⚡ Schedule Emails Automatically</h1><p>300 free emails/day • High deliverability • Never forget important emails</p>{% if not session.user_id %}<a href="/register" class="btn">🚀 Get Started Free</a>{% endif %}</div>
    <div class="container"><h2 style="text-align:center;margin-bottom:20px;color:#0369a1">💰 Simple Pricing Plans</h2><div class="pricing-grid">{% for name,plan in plans.items() %}<div class="card"><h3>{{plan.name}}</h3><div class="price">${{"%.2f"|format(plan.price)}}<small style="font-size:16px;">/mo</small></div><ul class="features"><li>📧 {{"{:,}".format(plan.emails_per_month)}} emails/month</li><li>⏰ {{plan.scheduled_emails}} scheduled emails</li><li>⚡ Brevo API powered</li>{% if plan.price>0 %}<li>🎯 Priority support</li><li>📈 Analytics</li>{% else %}<li>✨ 300 emails/day free</li><li>📧 100k contacts free</li>{% endif %}</ul></div>{% endfor %}</div></div>
</body>
</html>
'''
...
<div style="text-align:right;margin-bottom:20px"><a href="/test_brevo" class="test-btn">🔧 Test Brevo API</a></div>
...
if __name__ == '__main__':
    ...
    thread.start()
    print("=" * 60)
    print("🚀 Email Scheduler Pro - WITH ANIMATIONS")
    print("=" * 60)
    print("✅ Database initialized")
    print("✅ Background sender: ACTIVE (no duplicates)")
    print("✅ Server: http://127.0.0.1:5000")
    print("=" * 60)
    print("")
    print("⚙️  To use Brevo API, update:")
    print("   BREVO_CONFIG['api_key'] = 'your-api-key'")
    print("   BREVO_CONFIG['from_email'] = 'your-email'")
    print("")
    print("🎨 Features:")
    print("   • Smooth fade/slide animations")
    print("   • Hover effects on all cards")
    print("   • Glowing progress bars")
    print("   • Floating effect on cards")
    print("   • Pulse animation on logo")
    print("=" * 60)
