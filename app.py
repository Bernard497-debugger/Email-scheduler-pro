#!/usr/bin/env python3
"""
Auto Scheduled Email Sending App - WORKING ON RENDER
Using APScheduler which handles background tasks better
"""
...
# ==================== CONFIGURATION ====================
BREVO_API_KEY = 'xkeysib-YOUR-ACTUAL-API-KEY-HERE'  # CHANGE THIS
...
def check_and_send_emails():
    """This runs every 30 seconds via APScheduler"""
...
def schedule_email():
    ...
    try:
        ...
        flash(f'⏰ Auto-scheduler will send it (checks every 30 seconds)', 'info')
    except ValueError:
...
def scheduler_status():
    ...
    upcoming = c.execute('SELECT id, scheduled_time, recipient_email FROM scheduled_emails WHERE status = "scheduled" AND scheduled_time > ? ORDER BY scheduled_time LIMIT 5', (now.isoformat(),)).fetchall()
...
.hero h1{font-size:48px;color:#0369a1;margin-bottom:20px}
...
<body>
<div class="header"><div class="nav"><div class="logo">🚀 Email Scheduler Pro <span class="badge">APScheduler</span></div><div class="nav-links">{% if session.user_id %}<a href="/dashboard">Dashboard</a><a href="/logout">Logout</a>{% else %}<a href="/login">Login</a><a href="/register">Register</a>{% endif %}</div></div></div>
<div class="hero"><h1>⚡ Schedule Emails Automatically</h1><p>Works on Render! APScheduler running in background</p>{% if not session.user_id %}<a href="/register" class="btn">Get Started Free</a>{% endif %}</div>
<div class="container"><h2 style="text-align:center">Pricing Plans</h2><div class="pricing-grid">{% for name,plan in plans.items() %}<div class="card"><h3>{{plan.name}}</h3><div class="price">${{"%.2f"|format(plan.price)}}/mo</div><ul style="list-style:none"><li>📧 {{plan.emails_per_month}} emails/month</li><li>⏰ {{plan.scheduled_emails}} scheduled</li></ul></div>{% endfor %}</div></div>
</body>
</html>
'''
...
<div style="text-align:right;margin-bottom:20px"><a href="/test_brevo" class="test-btn">🔧 Test API</a> <a href="/scheduler_status" class="test-btn" style="background:#0369a1">📊 Scheduler Status</a></div>
...
def start_scheduler():
    ...
    scheduler.add_job(func=check_and_send_emails, trigger="interval", seconds=30)
    scheduler.start()
    print("=" * 50)
