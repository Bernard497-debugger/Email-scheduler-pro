...
Using Brevo API + Cron-job.org for reliable scheduling
No background threads - works perfectly on free tier
"""
...
# Database path - persists on Render with disk mount
DATABASE = os.environ.get('DATABASE_PATH', '/app/data/email_scheduler.db')
...
print(f"🚀 Starting Email Scheduler Pro on Render...")
...
def run_scheduler():
    """
    Manual/External trigger endpoint for email scheduler.
    Call this endpoint via cron-job.org or UptimeRobot every minute.
    ...
    try:
        ...
        due_emails = db.execute('''
            SELECT id, scheduled_time, recipient_email, subject, user_id 
        ...
        if not due_emails:
            return jsonify({
                "status": "success",
                "message": "No emails due at this time",
...
@app.route('/run-scheduler-protected')
@login_required
def run_scheduler_protected():
    """
    Protected version of the scheduler endpoint - requires login.
    Useful for manual testing from the dashboard.
    """
...
def scheduler_status():
    """
    Get current scheduler status and pending emails count.
    ...
    try:
        ...
        # Get next 5 due emails
        next_emails = db.execute('''
            SELECT id, recipient_email, subject, scheduled_time 
            FROM scheduled_emails 
            WHERE status = 'scheduled'
            ORDER BY scheduled_time ASC
            LIMIT 5
        ...
        return jsonify({
            ...
            "next_emails": next_emails_list,
            "scheduler_endpoint": "/run-scheduler",
            "cron_job_url": f"{request.base_url.replace('scheduler-status', 'run-scheduler')}"
        }), 200
...
@app.route('/force-send/<int:email_id>')
@login_required
def force_send_email(email_id):
    """
    Manually force-send a specific scheduled email (admin/debug feature).
    ...
    try:
        ...
        success = send_scheduled_email(email_id)
        ...
        if success:
            ...
            return jsonify({
                "status": "error",
                "message": f"Failed to send email {email_id}"
            }), 500
            
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

@app.route('/clear-failed-emails')
@login_required
def clear_failed_emails():
    """
    Clear/delete failed emails (admin feature).
    """
    user = get_user(session['user_id'])
    if not user:
        return jsonify({"status": "error", "message": "Not logged in"}), 401
    ...
    try:
        ...
        return jsonify({
            "status": "success",
            "message": f"Deleted {result.rowcount} failed emails"
        }), 200
...
<body>
    <div class="header">
        <div class="nav">
            ...
            <div class="nav-links">
                ...
                <a href="#" onclick="manualRunScheduler()" style="cursor: pointer;">🔄 Run Scheduler</a>
                <a href="{{ url_for('upgrade_plan') }}">Upgrade</a>
                <a href="{{ url_for('logout') }}">Logout</a>
            </div>
        </div>
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            <div class="stat-card" onclick="checkSchedulerStatus()" style="cursor: pointer;">
                <h3>⚙️ Scheduler Status</h3>
                <div class="number" id="schedulerStatus">Loading...</div>
                <small>Click to check/refresh</small>
            </div>
            <div class="stat-card">
                <h3>📊 Monthly Usage</h3>
        ...
        <div class="email-list">
            <div style="padding: 18px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; font-weight: bold;">
                📋 Scheduled Emails
                <button onclick="clearFailedEmails()" class="btn-sm btn-danger" style="float: right;">🗑️ Clear Failed</button>
            </div>
            {% if scheduled_emails %}
                    ...
                    <div class="email-item">
                        ...
                        {% if email.status == 'scheduled' %}
                            <div style="margin-top: 10px;">
                                ...
                                <a href="#" onclick="forceSendEmail({{ email.id }})" class="btn-sm btn-success" style="text-decoration: none;">⚡ Force Send</a>
                            </div>
                        {% endif %}
                    </div>
                {% endfor %}
    ...
    <script>
        function manualRunScheduler() {
            fetch('/run-scheduler')
                .then(response => response.json())
                .then(data => {
                    alert(`Scheduler executed!\\n\\nFound: ${data.emails_found} emails\\nSent: ${data.emails_sent}\\nFailed: ${data.emails_failed}`);
                    location.reload();
                })
                .catch(error => alert('Error running scheduler: ' + error));
        }
                ...
                .then(data => {
                    if (data.status === 'success') {
                        document.getElementById('schedulerStatus').innerHTML = 
                            `⏰ ${data.pending_emails_due} due / ${data.total_scheduled_emails} total`;
                        alert(`Scheduler Status:\\n\\nPending due: ${data.pending_emails_due}\\nTotal scheduled: ${data.total_scheduled_emails}\\nNext email: ${data.next_emails[0]?.scheduled_time || 'None'}`);
                    }
                })
                .catch(error => console.error('Error:', error));
        }
        
        function forceSendEmail(emailId) {
            if (confirm('Force send this email now?')) {
                fetch(`/force-send/${emailId}`)
                    .then(response => response.json())
                    .then(data => {
                        alert(data.message);
                        location.reload();
        ...
        function clearFailedEmails() {
            if (confirm('Delete all failed emails? This cannot be undone.')) {
                fetch('/clear-failed-emails')
                    .then(response => response.json())
                    .then(data => {
                        alert(data.message);
                        location.reload();
...
if __name__ == '__main__':
    print("=" * 60)
    print("🚀 Email Scheduler Pro - Render Optimized")
    print("=" * 60)
    print("✅ Database initialized")
    print("✅ Cron-triggered email sender ready")
    print("✅ Available endpoints:")
    print("   - /run-scheduler (for cron-job.org)")
    print("   - /scheduler-status (monitoring)")
    print("   - /health (health check)")
    print("=" * 60)
    print("")
    print("⚙️  To enable auto email sending:")
    print("   1. Go to cron-job.org")
    print("   2. Create a cron job")
    print("   3. URL: https://YOUR-APP.onrender.com/run-scheduler")
    print("   4. Set interval: Every 1 minute")
    print("")
    print("📧 Brevo API Setup:")
    print("   Set BREVO_API_KEY environment variable in Render dashboard")
    print("=" * 60)
