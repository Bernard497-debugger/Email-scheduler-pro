#!/usr/bin/env python3
"""
Auto Scheduled Email Sending App - Production Ready for Render
Using Brevo API - WITH BEAUTIFUL ANIMATIONS - DATABASE FIXED
"""
...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'pydroid-email-scheduler-secret-key-change-this')

DATABASE = 'email_scheduler.db'

# Brevo API Configuration from environment variables
BREVO_CONFIG = {
    'api_key': os.environ.get('BREVO_API_KEY', 'xkeysib-your-api-key-here'),
    'from_email': os.environ.get('BREVO_FROM_EMAIL', 'testing@brevo.com'),
...
def init_db():
    ...
    try:
        ...
        c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='scheduled_emails'")
        if not c.fetchone():
            print("Creating scheduled_emails table...")
...
def test_brevo():
    ...
    success, msg, _ = send_email_via_brevo(
        ...
        f'Test email sent at {datetime.now()}\n\nYour email scheduler is working perfectly! 🎉\n\nFeatures:\n• Auto-scheduling\n• 300 free emails/day\n• Beautiful email templates', 
        user['email']
    )
...
if __name__ == '__main__':
    ...
    print("✅ Background email sender started (no duplicates)")
    print("✅ Server running")
    print("=" * 60)
    print("")
    print("📧 Brevo API Status:")
    if BREVO_CONFIG['api_key'] and BREVO_CONFIG['api_key'] != 'xkeysib-your-api-key-here':
        print("   ✅ Brevo API configured")
    else:
        print("   ⚠️  Brevo API not configured - set BREVO_API_KEY env var")
    print("")
    print("🎨 Features enabled:")
    print("   • Beautiful animations")
    print("   • Auto-scheduling engine")
    print("   • Duplicate protection")
    print("   • Monthly usage limits")
    print("=" * 60)
