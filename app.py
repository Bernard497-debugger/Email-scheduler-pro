#!/usr/bin/env python3
"""
Email Scheduler Pro - PRODUCTION READY FOR RENDER
"""
...
BREVO_FROM_EMAIL = os.environ.get('BREVO_FROM_EMAIL', 'botsile55@gmail.com')
...
# Use /tmp for database on Render (persistent disk not available on free tier)
# For production with persistent storage, you'd need to use PostgreSQL
DATABASE = '/tmp/email_scheduler.db' if os.environ.get('RENDER') else 'email_scheduler.db'
...
def init_db():
    ...
    c.execute('CREATE INDEX IF NOT EXISTS idx_scheduled_time ON scheduled_emails(scheduled_time, status)')
...
def send_email_via_brevo(to_email, subject, body, user_email=None):
    ...
    if not BREVO_API_KEY:
        return False, "API key not configured. Please set BREVO_API_KEY environment variable.", None
    ...
    try:
        ...
        data = {
            'sender': {'email': BREVO_FROM_EMAIL, 'name': 'Email Scheduler Pro'},
            'to': [{'email': to_email}],
            'subject': subject,
            'textContent': body,
...
def send_due_emails():
    """Check and send due emails - runs in background thread"""
    try:
        db = sqlite3.connect(DATABASE)
        db.row_factory = sqlite3.Row
        c = db.cursor()
        ...
        print(f"Error in send_due_emails: {e}")
...
# Start scheduler thread (only in production, not in debug mode)
scheduler_started = False

def start_scheduler():
    global scheduler_started
    if not scheduler_started and not app.debug:
...
def test_brevo():
    ...
    success, msg, msg_id = send_email_via_brevo(
        ...
        f'Test email sent at {datetime.now().strftime("%H:%M:%S")}\n\nYour API key is working perfectly! 🎉',
        user['email']
    )
    
    if success:
        flash(f'✅ Test email sent! Check your inbox.', 'success')
    else:
        flash(f'❌ Failed: {msg}', 'error')
...
<div class="hero"><h1>⚡ Schedule Emails Automatically</h1><p>Powered by Brevo API ✓ Auto-scheduler Active ✓</p>{% if not session.user_id %}<a href="/register" class="btn">Get Started Free</a>{% endif %}</div>
...
<!DOCTYPE html>
<html>
<head><title>Dashboard</title><meta name="viewport" content="width=device-width, initial-scale=1"><style>
*{margin:0;padding:0}
body{font-family:Arial;background:#f0f9ff}
.header{background:white;padding:15px 20px;box-shadow:0 2px 5px rgba(0,0,0,0.1)}
.nav{display:flex;justify-content:space-between;max-width:1200px;margin:0 auto;flex-wrap:wrap}
...
.test-btn{background:#10b981;padding:5px 10px;border-radius:5px;color:white;text-decoration:none;display:inline-block;margin:0 5px}
...
<div style="text-align:right;margin-bottom:20px"><a href="/test_brevo" class="test-btn">🔧 Test API</a> <a href="/health" class="test-btn" style="background:#0369a1">📊 Status</a></div>
<div class="stats"><div class="stat-card"><h3>Monthly Usage</h3><div>{{user.emails_sent_this_month}} / {{plan.emails_per_month}}</div><div style="background:#e2e8f0;border-radius:10px;margin-top:10px"><div style="width:{{usage_percentage}}%;height:10px;background:#0284c7;border-radius:10px"></div></div><small>{{remaining_emails}} remaining</small></div><div class="stat-card"><h3>Scheduled</h3><div>{{remaining_scheduled}} / {{plan.scheduled_emails}}</div><small>slots available</small></div></div>
...
<body><div class="container"><h2 style="text-align:center">Upgrade Plan</h2><div class="pricing-grid">{% for name,plan in plans.items() %}<div class="card {% if name==current_plan %}current{% endif %}"><h3>{{plan.name}}</h3><div class="price">${{"%.2f"|format(plan.price)}}/mo</div><ul style="list-style:none;padding:0"><li>📧 {{plan.emails_per_month}} emails/month</li><li>⏰ {{plan.scheduled_emails}} scheduled</li></ul>{% if name==current_plan %}<button disabled>Current</button>{% else %}<a href="/change_plan/{{name}}" class="btn">Switch</a>{% endif %}</div>{% endfor %}</div><div style="text-align:center;margin-top:30px"><a href="/dashboard">← Back to Dashboard</a></div></div></body>
</html>
'''
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("🚀 Email Scheduler Pro - READY FOR RENDER")
    print(f"✅ API Key: {'CONFIGURED' if BREVO_API_KEY else '❌ MISSING'}")
    print(f"✅ Auto-scheduler: ACTIVE")
